...
function DashboardLayoutContent({
  ...
  const currentIndex = useMemo(() => menuItems.findIndex(item => item.path === location), [location, menuItems]);
  ...
  const goNext = useCallback(() => { if (nextPage) setLocation(nextPage.path); }, [nextPage, setLocation]);
  const goPrev = useCallback(() => { if (prevPage) setLocation(prevPage.path); }, [prevPage, setLocation]);
  const goHome = useCallback(() => setLocation("/dashboard"), [setLocation]);
  ...
  return (
    <>
      ...
      <SidebarInset>
        ...
        {currentIndex >= 0 && (
          <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 flex items-center gap-1 bg-background/90 backdrop-blur-lg border border-border/50 rounded-full px-2 py-1.5 shadow-lg">
            <button
              onClick={goPrev}
              disabled={!prevPage}
              title={prevPage?.label}
              className="flex items-center gap-1 px-2.5 py-1.5 rounded-full text-xs font-medium transition-all disabled:opacity-30 disabled:cursor-not-allowed hover:bg-accent"
            >
              <ArrowRight className="h-3.5 w-3.5" />
              {prevPage && !isMobile && <span className="max-w-[80px] truncate">{prevPage.label}</span>}
            </button>
            ...
            <button
              onClick={goHome}
              title="لوحة التحكم"
              className="flex items-center gap-1 px-2.5 py-1.5 rounded-full text-xs font-medium transition-all hover:bg-accent"
            >
            ...
            {showScrollTop && (
              <>
                <button
                  onClick={scrollToTop}
                  title="العودة للأعلى"
                  className="flex items-center px-2 py-1.5 rounded-full text-xs transition-all hover:bg-accent"
                >
            ...
            <button
              ...
              className="flex items-center gap-1 px-2.5 py-1.5 rounded-full text-xs font-medium transition-all disabled:opacity-30 disabled:cursor-not-allowed hover:bg-accent"
            >
              {nextPage && !isMobile && <span className="max-w-[80px] truncate">{nextPage.label}</span>}
